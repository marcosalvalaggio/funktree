local function clang(root_lines, buf)
	...
	for i, line in ipairs(root_lines) do
		...
		if typedef_ongoing then
			if typedef_name then
				...
				if typedef_struct_position then
					table.insert(
						reduced_lines,
						string.format("struct: %s, line: %d", typedef_name, typedef_struct_position)
					)
					status = true
				elseif typedef_enum_position then
					table.insert(
						reduced_lines,
						string.format("enum: %s, line: %d", typedef_name, typedef_enum_position)
					)
					status = true
				end
			end
		...
		elseif class_name then
			...
			if
				method_name
				and method_name ~= "struct"
				and method_name ~= "enum"
				and string.sub(method_name, 1, 3) ~= "new"
			then
				...
				if
					function_name
					and function_name ~= "struct"
					and function_name ~= "enum"
					and string.sub(function_name, 1, 3) ~= "new"
				then
